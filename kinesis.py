...
class KinesisMotor(KinesisDevice):
    ...
    Added Args:
        posCur: Current position of actuator in encoder counts, pos1: Previous
        position of actuator (during coarse alignment), pos2: New position of
        actuator (during coarse alignment), name: Label for actuator/axis,
        status: Individual alignment status of actuator (during coarse
        alignment)
    """
    def __init__(self, conn, scale="step", posCur=0, pos1, pos2, name, status):
        KinesisDevice.__init__(self,conn)
